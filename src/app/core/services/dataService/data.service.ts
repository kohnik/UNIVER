...
export class DataService {
  ...
  getData(startDate: number, endDate: number, region: string): Observable<DataForCharts> {
    const paramsForImageReq = `http://localhost:8080/api/v1/registrations?before=${startDate}&after=${endDate}&region=${region}`;
    return this.http.get<DataForCharts>(paramsForImageReq)
  }

  getExtremum(startDate: number, endDate: number): Observable<Extremum>  {
    const paramsForImageReq = `http://localhost:8080/api/v1/registrations/extremum?before=${startDate}&after=${endDate}`;
